...
* **Deterministic Payment-State Authority Model:** Razorpay API/payment state is strictly authoritative for financial/payment state. Merchant records represent merchant belief. Webhooks represent event-delivery observations. AI never decides or overrides authoritative payment state.
* **Deterministic AI Gate:** AI never decides whether AI should run. Activation is governed strictly by deterministic code rules.
* **Deterministic Claim Verifier:** Every claim generated by Gemini must cite valid evidence IDs present in the evidence package and cannot contradict authoritative payment state. Unsupported claims are flagged as `REJECTED`/`CONTRADICTED`.
...
* **MCP Safety Boundary & No Autonomous Actions:** PayTrace is purely diagnostic and evidence-oriented. AI and MCP interfaces NEVER have access to money-moving or financial-mutating operations (no capture, refund, payout, transfer, or link mutation) and cannot execute arbitrary shell/SQL.
